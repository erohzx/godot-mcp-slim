...
import { normalizeParameters, convertCamelToSnakeCase, validatePath, createErrorResponse, isGodot44OrLater, } from './utils.js';
...
class GodotServer {
    ...
    */
    resolveProjectPath(projectPath, contextPath) {
     ...
     */
    async sendGameCommand(command, params = {}, timeoutMs = 10000) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.gameConnection.pendingResolve = (response) => {
    ...
    async gameCommand(name, args, argsFn, timeoutMs) {
        if (!this.activeProcess)
        ...
        try {
            ...
            if (response.error)
                return createErrorResponse(`${name} failed: ${response.error}`);
    ...
    async headlessOp(operation, args, argsFn) {
        args = normalizeParameters(args || {});
    ...
    async handleLaunchEditor(args) {
        ...
        if (!resolvedPath) {
            return createErrorResponse('Project path is required and could not be auto-detected. Provide a path to a Godot project or any file within one.');
        }
    ...
    async handleCreateScene(args) {
        ...
        if (!resolvedPath) {
            return createErrorResponse('Project path is required and could not be auto-detected. Provide projectPath or ensure scenePath is within a Godot project.');
        }
        args.projectPath = resolvedPath;
        if (!args.scenePath) {
    ...
    async handleRenameFile(args) {
        ...
        if (!validatePath(args.projectPath) || !validatePath(args.filePath) || !validatePath(args.newPath))
            return createErrorResponse('Invalid path.');
        const projectFile = join(args.projectPath, 'project.godot');
        if (!existsSync(projectFile))
            return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
    ...
    async handleManageShader(args) {
        ...
        if (!validatePath(args.projectPath) || !validatePath(args.shaderPath))
            return createErrorResponse('Invalid path.');
        const projectFile = join(args.projectPath, 'project.godot');
        if (!existsSync(projectFile))
            return createErrorResponse(`Not a valid Godot project: ${args.projectPath}`);
    ...
    async handleManageProjectSettings(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
        if (action === 'read') {
    ...
    async handleGameMouseInput(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
        switch (action) {
            case 'click':
    ...
    async handleGameKeyboardInput(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
        if (action === 'gamepad') {
    ...
    async handleGameNodeProperty(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
        if (action === 'get') {
    ...
    async handleGameNodeMethod(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
        switch (action) {
            case 'call':
    ...
    async handleGameSceneManagement(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
        switch (action) {
            case 'change_scene':
    ...
    async handleGameAnimation(args) {
        args = normalizeParameters(args || {});
    ...
    async handleGamePhysicsQuery(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
    ...
    async handleGameUiContainer(args) {
        args = normalizeParameters(args || {});
        const { action, nodePath } = args;
        if (!nodePath)
            return createErrorResponse('nodePath is required');
    ...
    async handleGameRendering(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
    ...
    async handleGameNavigation(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
    ...
    async handleGameNetworking(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
    ...
    async handleGameSerialization(args) {
        args = normalizeParameters(args || {});
        const { action } = args;
