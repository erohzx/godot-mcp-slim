...
# --- Eval: Universal freeze-proof execution (PROJECT-AGNOSTIC) ---
var _eval_running: bool = false

func _cmd_eval(params: Dictionary) -> void:
	if _eval_running:
		_send_response({"error": "Eval already running, please wait"})
		return
	_eval_running = true
	...
	# --- SYSTEMATIC PRE-EXECUTION VALIDATION ---
	const MAX_CODE_LENGTH: int = 10000
	if code.length() > MAX_CODE_LENGTH:
		_eval_running = false
		_send_response({"error": "Code exceeds max length (%d chars)" % MAX_CODE_LENGTH})
		return
	
	# Block known freeze-causing patterns
	var dangerous_patterns: Array[String] = [
		"while true", "while( true )", "while(true)",
		"for i in range(1000000)", "for i in range(999999)",
		"OS.execute(", "Thread.new(", "OS.delay_msec(",
		"load_threaded_request(", "await get_tree().create_timer(", # Block nested awaits without yield
		"JSON.parse(", "FileAccess.open(", "DirAccess.open(" # Sync I/O
	]
	for pattern in dangerous_patterns:
		if code.find(pattern) != -1:
			_eval_running = false
			_send_response({"error": "Blocked dangerous pattern: '%s'. Use async/await patterns or safe APIs." % pattern})
			return
	...
	var script_source: String = (
		...
		+ "func execute():\n"
		+ "\tvar ret = _run()\n"
		+ "\tif ret != null and ret is Object and ret.has_method(\"resume\"):\n"
		+ "\t\tresult = await ret\n"
		+ "\telse:\n"
		+ "\t\tresult = ret\n"
		+ "\tdone = true\n"
	...
	if script.reload() != OK:
		_eval_running = false
		_send_response({"error": "GDScript compilation failed", "code_preview": code.left(100)})
		return
	
	var temp_node: Node = Node.new()
	temp_node.name = "Eval_%d" % Time.get_ticks_msec()
	...
	if not temp_node.has_method("execute"):
		...
		_eval_running = false
		_send_response({"error": "Internal error: execute() method not found"})
		return
	
	# --- HARD WATCHDOG & RECOVERY ---
	...
	# Yield once to ensure engine state is stable
	await get_tree().process_frame
	
	# Execute (this may block the main thread if code is synchronous)
	temp_node.execute()
	
	# Poll until done or timeout
	while not temp_node.done:
	...
	var elapsed_ms: int = int((Time.get_ticks_msec() / 1000.0 - start_time) * 1000.0)
	var response: Dictionary
	
	if temp_node.done:
		response = {
			"success": true,
			"result": _variant_to_json(temp_node.result),
			"time_ms": elapsed_ms
		}
	else:
		response = {
			"error": "Execution timed out after 5s. Code may be blocking or contain infinite loops.",
			"timeout": true,
			"time_ms": elapsed_ms,
			"suggestion": "Refactor to use 'await get_tree().process_frame' or break heavy tasks across frames."
		}
...
# --- Helper: Get all autoload names dynamically ---
func _get_autoload_names() -> Array[String]:
	var names: Array[String] = []
	var property_list = ProjectSettings.get_property_list()
	for prop in property_list:
		var name: String = prop.name
		if name.begins_with("autoload/"):
			var node_name = name.trim_prefix("autoload/")
...
func _cmd_wait(params: Dictionary) -> void:
	...
	# RANGE FIX: Use range() to iterate over an integer
	for i in range(frames):
		await get_tree().process_frame
	_send_response({"success": true, "waited_frames": frames})


func _variant_to_json(value: Variant, max_depth: int = 8, current_depth: int = 0, seen_objects: Dictionary = {}) -> Variant:
	...
	# Handle Objects with cycle detection
	if value is Object:
		var obj_id: int = value.get_instance_id()
		if seen_objects.has(obj_id):
			return {"_circular_ref": true, "class": value.get_class(), "id": obj_id}
		seen_objects[obj_id] = true
		
		if value is Node:
			var n: Node = value as Node
			return {"_type": "Node", "class": n.get_class(), "name": n.name, "path": str(n.get_path())}
		if value is Resource:
			var r: Resource = value as Resource
			return {"_type": "Resource", "class": r.get_class(), "path": r.resource_path}
		return {"_type": "Object", "class": value.get_class(), "id": obj_id}
	
	if value is Dictionary:
		var dict: Dictionary = {}
		for key in value:
			dict[str(key)] = _variant_to_json(value[key], max_depth, current_depth + 1, seen_objects)
		return dict
	
	if value is Array:
		var arr: Array = []
		for item in value:
			arr.append(_variant_to_json(item, max_depth, current_depth + 1, seen_objects))
		return arr
	
	# ... (keep your existing PackedArray, Vector, Transform, etc. handlers here, they don't need changes) ...
